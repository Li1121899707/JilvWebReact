      ...
      <h1>问题线索登记表</h1>
        <Form>
          <table>
            <tbody>
                <tr>
                  <td>线索来源</td>
                  <td colspan="3">
                    <div> {{formData.wenTiXianSuo_xianSuoLaiYuan}} </div>
                  </td>
                  <td>线索编号</td>
                  <td colspan="3">
                    <div> {{formData.wenTiXianSuo_xuHao}} </div>
                  </td>
                </tr>
                <tr>
                  <td>收件日期</td>
                  <td>
                    <div> {{formData.wenTiXianSuo_shouDaoShiJian}} </div>
                  </td>
                  <td>反映人</td>
                  <td>
                    <div> {{formData.wenTiXianSuo_fanYingRen}} </div>
                  </td>
                  <td >性别</td>
                  <td >
                    <div> {{formData.wenTiXianSuo_xingBie}} </div>
                  </td>
                </tr>
                <tr>
                  <td>政治面貌</td>
                  <td>
                    <div> {{formData.wenTiXianSuo_zhengZhiMianMao}} </div>
                  </td>
                  <td>联系电话</td>
                  <td>
                    <div> {{formData.wenTiXianSuo_dianHua}} </div>
                  </td>
                  <td>通信地址</td>
                  <td>
                    <div> {{formData.wenTiXianSuo_diZhi}} </div>
                  </td>
                </tr>
                <tr>
                  <td>工作单位</td>
                  <td colspan="2">
                    <div> {{formData.wenTiXianSuo_fanYingRenDanWei}} </div>
                  </td>
                  <td>职务</td>
                  <td colspan="2">
                    <div> {{formData.wenTiXianSuo_fanYingRenZhiWu}} </div>
                  </td>
                </tr>
                <tr>
                  <td>被反映人</td>
                  <td colspan="5">
                    <div> {{formData.wenTiXianSuo_beiFanYingRen}} </div>
                  </td>
                </tr>
                <tr>
                  <td>工作单位</td>
                  <td colspan="2">
                    <div> {{formData.wenTiXianSuo_beiFanYingRenDanWei}} </div>
                  </td>
                  <td>职务</td>
                  <td colspan="2">
                    <div> {{formData.wenTiXianSuo_beiFanYingRenZhiWu}} </div>
                  </td>
                </tr>
                <tr>
                  <td>反映主要问题</td>
                  <td colspan="5">
                    <div> {{formData.wenTiXianSuo_fanYingZhuYaoWenTi}} </div>
                  </td>
                </tr>
                <tr>
                  <td>拟办意见</td>
          ...
          <FormItem prop="wenTiXianSuo_chuLiFangShi" label="办理方式" label-width="220">
            <RadioGroup v-model="formData.wenTiXianSuo_chuLiFangShi">
          ...
          <div v-if="formData.wenTiXianSuo_chuLiFangShi === '自办'">
          ...
          <div v-if="formData.wenTiXianSuo_chuLiFangShi !== '自办' && formData.wenTiXianSuo_chuLiFangShi !== ''">
          ...
          <div v-if="formData.wenTiXianSuo_chuLiFangShi === '督办' || (formData.wenTiXianSuo_chuLiFangShi === '协办' && formData.wenTiXianSuo_niBanYiJian !== '')">
          ...
          <FormItem prop="wenTiXianSuo_shenPiLingDao" label="批办领导(交付于纪检监察室)" label-width="220">
            <Select v-model="formData.wenTiXianSuo_shenPiLingDao" style="width:220px">
    ...
    export default {
        ...
        methods: {
            ...
            submit(){
                ...
                // 加入formData未包含的数据
                let tempValues = {wenTiXianSuo_status :'已登记'}
                this.wenTiXianSuo_chuZhiFangShi!==''?(tempValues.wenTiXianSuo_chuZhiFangShi=this.wenTiXianSuo_chuZhiFangShi):''
                this.wenTiXianSuo_chuLiFangShi_buMen!==''?(tempValues.wenTiXianSuo_chuLiFangShi_buMen=this.wenTiXianSuo_chuLiFangShi_buMen):''
                this.wenTiXianSuo_chuLiFangShi_neiRong!==''?(tempValues.wenTiXianSuo_chuLiFangShi_neiRong=this.wenTiXianSuo_chuLiFangShi_neiRong):''
                this.wenTiXianSuo_chuLiFangShi_shiJian!==''?(tempValues.wenTiXianSuo_chuLiFangShi_shiJian=this.wenTiXianSuo_chuLiFangShi_shiJian):''

                // 合并formData和未包含数据
                let values = Object.assign(this.formData, tempValues);
                ...
                // 执行添加
                var url = `thread/claimAndComplete?taskId=${taskid}&processInstanceId=${this.$route.params.id}&nextAssignee=${this.formData.wenTiXianSuo_shenPiLingDao}&isLocal=${0}`
