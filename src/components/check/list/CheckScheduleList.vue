...
<div>
    ...
    <Table height="500" :columns="scheduleListColumnsWithCheck" :data="dataSource"></Table>
...
import {get} from '@/utils/http'
    ...
    export default {
        data () {
            return {
                scheduleListColumnsWithCheck: GLOBAL.scheduleListColumnsWithCheck,
                dataSource:[],
                // 线索反映问题类型 select 框
        ...
        methods: {
            search(){
                var url = 'activiti/process/instances/all?processDefinitionKey=' + GLOBAL.processDefinitionKey + '&search=,status=初步核实,,isChuBuHeShi_yueDuJinDu=true'
                get(url, {
                    size: 10,
