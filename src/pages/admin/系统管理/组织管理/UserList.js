...
import { Form, Button, Divider, Icon, Input, Layout, Select, Table, Tag, Tooltip, notification, Modal, message } from 'antd'
...
class UserList extends Component {
  ...
  resetPassword = id => {
    put('sys/user-employees/reset-password', id, params).then(res => {
  ...
  tyConfirm = userId => {
    ...
    confirm({
      ...
      cancelText: '取消',
      onOk() {
        put('sys/user-employees/disable', userId, params).then(res => {
          that.fetch()
          notification.success({
            message: '停用成功'
          })
  ...
  qyConfirm = userId => {
    ...
    confirm({
      ...
      cancelText: '取消',
      onOk() {
        put('sys/user-employees/enable', userId, params).then(res => {
          that.fetch()
          notification.success({
            message: '启用成功'
          })
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: '角色名称',
        dataIndex: 'userInfo.roleList[0].roleName'
      },
      {
        title: '电子邮箱',
        ...
        render: (text, record) => {
          let tt = (
            <div>
              ...
              {/*  <Tag*/}
              {/*    onClick={() => {*/}
              {/*      router.push('/admin/system/organ/userfenpei')*/}
              {/*    }}*/}
              {/*  >*/}
              {/*    分配角色*/}
              {/*  </Tag>*/}
              ...
              {/*  <Tag*/}
              {/*    onClick={() => {*/}
              {/*      router.push('/admin/system/organ/userShuJu')*/}
              {/*    }}*/}
              {/*  >*/}
              {/*    数据权限*/}
              {/*  </Tag>*/}
          ...
          const bj_bt = (
            <Tooltip placement='top' title='编辑用户'>
              <Button
                ...
                style={{ paddingLeft: 0 }}
                onClick={() => {
                  router.push({ pathname: `/admin/system/organ/useredit/${record.id}` })
                }}
              >
          ...
          <Content className={styles.contentbox}>
            <div className={styles.header}>
              ...
              <Button
                ...
                onClick={() => {
                  router.push('/admin/system/organ/useradd') // 跳转方式 2
                }}
              >
                <Icon type='plus' />
                新增
