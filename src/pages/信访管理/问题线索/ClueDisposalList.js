...
class ClueDisposalList extends Component {
  ...
  endTask = record => {
    let val = {}
    const { wenTiXianSuo_chuZhiFangShi, flow_path } = record.form
    val.wenTiXianSuo_status = '已上会'
    if (wenTiXianSuo_chuZhiFangShi === '谈话函询') {
      val.tanHuaHanXun_status = '待填谈话函询'
      val.status = '谈话函询'
      val.flow_path = `${flow_path},谈话函询`
    } else if (wenTiXianSuo_chuZhiFangShi === '初步核实') {
      val.chuBuHeShi_status = '待填初核申请表'
      val.status = '初步核实'
      val.flow_path = `${flow_path},初步核实`
    }
  ...
  renderBtn = record => {
    const { wenTiXianSuo_status, wenTiXianSuo_chuLiFangShi, wenTiXianSuo_chuZhiFangShi } = record.form
    ...
    if (this.isPermissionBtn(record)) {
      ...
      if (wenTiXianSuo_status === '已登记' || wenTiXianSuo_status === '审批中') {
        operateBtn = {
          link: `/admin/petition/clue/add/${processInstanceId}/lingdaoshenpi`,
          label: '审批'
        }
      } else if (wenTiXianSuo_status === '已上会' && (wenTiXianSuo_chuLiFangShi === '协办' || wenTiXianSuo_chuLiFangShi === '督办')) {
        operateBtn = {
          link: `/admin/petition/clue/${processInstanceId}/dubanxiebanresult`,
          label: '登记办理结果'
        }
        ...
        operateBtn = {
          link: `/admin/petition/clue/${processInstanceId}/xiansuochuzhi`,
          label: '上会'
        }
      }
      // else if (wenTiXianSuo_status === '已审批' && wenTiXianSuo_chuLiFangShi === '自办') {
      //   operateBtn = {
      //     link: `/admin/petition/clue/${processInstanceId}/xiansuochuzhi`,
      //     label: '上会'
      //   }
      // }
        ...
        operateBtn = {
          link: `/admin/petition/clue/${processInstanceId}/zancun`,
          label: '填写暂存待查呈批表'
        }
      } else if (wenTiXianSuo_status === '予以了结已登记' || wenTiXianSuo_status === '予以了结审批中') {
        operateBtn = {
          link: `/admin/petition/clue/add/${processInstanceId}/YuYiLiaoJieLingDaoShenPi`,
          label: '审批'
        }
      } else if (wenTiXianSuo_status === '暂存待查已登记' || wenTiXianSuo_status === '暂存待查审批中') {
        operateBtn = {
          link: `/admin/petition/clue/add/${processInstanceId}/ZanCunLingDaoShenPi`,
          label: '审批'
        }
    ...
    if (wenTiXianSuo_status === '已登记' || wenTiXianSuo_status === '审批中') {
      ...
      chaKanBtn = {
        link: `/admin/petition/clue/问题线索/${record.processInstanceId}/xiansuodetail`,
        label: '查看'
      }
    } else if (wenTiXianSuo_status === '已审批') {
      chaKanBtn = {
      ...
      chaKanBtn = {
        link: `/admin/petition/clue/show/${processInstanceId}/YuYiLiaoJieLingDaoShenPi`,
        label: '查看'
      }
    } else if (wenTiXianSuo_status === '暂存待查已登记' || wenTiXianSuo_status === '暂存待查审批中') {
      chaKanBtn = {
      ...
      chaKanBtn = {
        link: `/admin/petition/clue/show/${processInstanceId}/dengJiJieGuo`,
        label: '查看'
      }
    } else if (wenTiXianSuo_status === '予以了结已审批' || wenTiXianSuo_status === '暂存待查已审批') {
      chaKanBtn = {
  ...
  process = () => {
    ...
    if (this.monthly) {
      ...
      let tanHuaArr = []
      if (this.record.form.wenTiXianSuo_yueDuJinDu) {
        tanHuaArr = this.record.form.wenTiXianSuo_yueDuJinDu
      }
      const obj = {
      ...
      tanHuaArr.push(obj)
      val.isWenTiXianSuo_yueDuJinDu = 'true'
      val.wenTiXianSuo_yueDuJinDu = tanHuaArr
  ...
  export = () => {
    ...
    if (keys.length === 0) {
      notification.info({ message: '请选择需要导出上会的材料的项目' })
    } else {
  ...
  render() {
    ...
    const rowSelection = {
      ...
      getCheckboxProps: record => ({
        disabled: record.form.wenTiXianSuo_status !== '已审批', // Column configuration not to be checked
        name: record.name
      })
    }
