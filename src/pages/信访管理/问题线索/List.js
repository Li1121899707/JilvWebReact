...
class List extends Component {
  ...
  getTongJiInfo = () => {
    get('thread/doStatistic').then(res => {
      this.setState({ tongJiInfo: res.data })
    })
  }
                ...
                <table className='query-condition-tb'>
                  <tbody style={{ textAlign: 'right' }}>
                    ...
                    <tr>
                      ...
                      <td>
                        <Form.Item label='反映人'>{getFieldDecorator('wenTiXianSuo_fanYingRen~', {})(<Input allowClear />)}</Form.Item>
                      </td>
                      <td>
          ...
          <Alert
            ...
            message={
              <span>
                ...
                转办/交办:
                <span style={{ color: 'red' }}>{tongJiInfo.transAndAssign || 0}</span>件, 办结
                <span style={{ color: 'red' }}>{tongJiInfo.transEnd || 0}</span>件, 办理中
                <span style={{ color: 'red' }}>{tongJiInfo.transNotEnd || 0}</span>件
                <br />
                督办:
                <span style={{ color: 'red' }}>{tongJiInfo.supervise || 0}</span>件, 办结
                <span style={{ color: 'red' }}>{tongJiInfo.supEnd || 0}</span>件, 办理中
                <span style={{ color: 'red' }}>{tongJiInfo.supNotEnd || 0}</span>件<br />
                协办:<span style={{ color: 'red' }}>{tongJiInfo.assist || 0}</span>
                件, 办结
                <span style={{ color: 'red' }}>{tongJiInfo.assistEnd || 0}</span>件, 办理中
