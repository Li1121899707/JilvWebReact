...
<div>
        ...
        <Form inline label-position="right" :label-width="200">
            ...
            <FormItem label="办理状态">
                <Select v-model="tanHuaHanXun_status" style="width:200px" clearable>
                    <Option v-for="item in liAnZhunBeiStatus" :value="item.value" :key="item.value">{{ item.label }}</Option>
                </Select>
            </FormItem>
        </Form>
    </Col>
    ...
    <Row style="padding-bottom:20px">
        <Col span="3">
            <Button type="success">批量导出</Button>
        </Col>
    </Row>
    
    <Table height="500" :columns="talkScheduleListColumns" :data="dataSource"></Table>
...
import {get} from '@/utils/http'
    ...
    export default {
        data () {
            return {
                talkScheduleListColumns: GLOBAL.talkScheduleListColumns,
                dataSource: [],
                // 线索反映问题类型 select 框
                problemType: GLOBAL.problemType,
                // 办理状态 select 框
                liAnZhunBeiStatus: GLOBAL.liAnZhunBeiStatus,
                // 办理方式
                ...
                isTanHuaHanXun_yueDuJinDu: true,
                dataCount:0,
                dataPage:1,
            }
        },
        methods: {
            search(){
                var url = 'activiti/process/instances/all?processDefinitionKey=' + GLOBAL.processDefinitionKey + '&search=,status=审查调查,isShenChaDiaoCha_yueDuJinDu=true'
                get(url, {
                    size: 5,
