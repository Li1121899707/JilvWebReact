...
class LiAnShenChaShenPiTable extends Component {
  constructor(props) {
    super(props)
    this.state = {
      dataSource: {},
    ...
    this.statusKey = 'LiAnShenCha_fangAn'
  }
  ...
  fetch = () => {
    get(`activiti/process/instance?processInstanceId=${this.id}`).then(res => {
      ...
      const leaderOption_shenChaDiaoCha = concatForArr(res.data.form, ['shenChaDiaoCha_ChengPi', 'shenChaDiaoCha_fangAnChengPi'])
      this.setState(
        {
  ...
  submit = () => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      if (taskGroup === 'LiAnShenCha_fangAn_dangWeiShuJi') {
        values.shenChaDiaoCha_status = '立案审查方案已审批'
        values.shenChaDiaoCha_yanQi_childstatus = '工作延期呈批表未申请'
        values.shenChaDiaoCha_waiCha_childstatus = '外查工作呈批表未申请'
        values.shenChaDiaoCha_childstatus = '立案中'
      } else {
        values.shenChaDiaoCha_status = '立案审查方案审批中'
      }
      ...
      if (err) return false
      let yijianArr = []
      if (this.state.dataSource.shenChaDiaoCha_fangAnChengPi) {
        yijianArr = this.state.dataSource.shenChaDiaoCha_fangAnChengPi
      }
      const obj = {
        ...
        <div className={styles.content_box}>
          ...
          <Form>
            ...
            <p style={{ textAlign: 'left' }}>
              相关附件:
              {dataSource.shenChaDiaoCha_fangAnChengPi_files &&
                dataSource.shenChaDiaoCha_fangAnChengPi_files.map(item => (
                  <a
                    target='_blank'
                    onClick={() => {
...
const wapper = Form.create()(LiAnShenChaShenPiTable)
