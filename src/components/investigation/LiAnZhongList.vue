...
<div>
        ...
        <Form inline  label-position="right" :label-width="200">
            ...
            <FormItem label="办理状态">
                <Select v-model="shenChaDiaoCha_status" style="width:200px" clearable>
                    <Option v-for="item in liAnZhongStatus" :value="item.value" :key="item.value">{{ item.label }}</Option>
                </Select>
            </FormItem>
        </Form>
    </Col>
    ...
    <Table height="600" :columns="investigationLiAnZhongListColumns" :data="dataSource"></Table>
...
import {get} from '@/utils/http'
    ...
    export default {
        data () {
            return {
                investigationLiAnZhongListColumns: GLOBAL.investigationLiAnZhongListColumns,
                dataSource: [],
                // 线索反映问题类型 select 框
                ...
                // 办理方式
                liAnZhongStatus: GLOBAL.liAnZhongStatus,
                // 线索来源
                ...
                status:'审查调查',// status=%E5%AE%A1%E6%9F%A5%E8%B0%83%E6%9F%A5%EF%BC%8CshenChaDiaoCha_childstatus=%E7%AB%8B%E6%A1%88%E5%87%86%E5%A4%87
                shenChaDiaoCha_childstatus: '立案准备',
                dataCount:0,
                dataPage:1,
            }
        },
        methods: {
            search(){
                var url = 'activiti/process/instances/all?processDefinitionKey=' + GLOBAL.processDefinitionKey + '&search=,status=审查调查，shenChaDiaoCha_childstatus=立案中'
                get(url, {
                    size: 10,
