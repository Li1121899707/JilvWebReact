...
<div>
    <Col span="22">
        <Form inline  label-position="right" :label-width="200">
            <FormItem label="导入时间起">
                <DatePicker @on-change="startTime=$event" type="date" placeholder="请选择日期" style="width: 200px"></DatePicker>
            </FormItem>
            <FormItem label="导入时间止">
                <DatePicker @on-change="endTime=$event" type="date" placeholder="请选择日期" style="width: 200px"></DatePicker>
            </FormItem>
            <FormItem label="反映人">
    ...
    <Row style="padding-bottom:20px">
        <Col span="3">
            <Button type="success" v-on:click="add">登记新的信访件</Button>
        </Col>
        <Col span="3">
            <Button type="success">批量导入信访件(未完成)</Button>
        </Col>
    </Row>
    
    <Table height="550" :columns="lettersListColumns" :data="dataSource"></Table>
...
import {get} from '@/utils/http'
    export default {
                                    ...
                                    h('span', {
                                        ...
                                        on: {
                                            click: () => {
                                                ...
                                                this.$router.push({ path: 'letters-show' })
                                            }
                                        }
                                    },'详情'),
                                    h('span', {
                                        ...
                                        on: {
                                            click: () => {
                                                ...
                                                this.$store.commit('updateXinfangjianIdHandle', params.row.id)
                                                this.$router.push({ path: 'letters-update' })
                                            }
                                        }
                                    },'编辑'),
                                    h('span', {
        ...
        methods: {
            ...
            search(){
                //var url = 'petitions/search?startTime=&endTime=2020-04-17T00%3A00%3A00.000Z&reporter=&source=&state=&informee='
                
                var url = 'petitions/search?startTime='+this.startTime+'&endTime='+this.endTime+'&reporter='+this.reporter+'&source='+this.source+'&state='+this.state+'&informee='+this.informee
                get(url, {
                    size: 10,
                    page: this.dataPage-1,
                }).then(res => {
            ...
            add(){
                this.$router.push({ path: 'letters-add' })
            }
        }
    }
</script>